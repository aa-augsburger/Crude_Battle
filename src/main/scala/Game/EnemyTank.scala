...
class EnemyTank extends Tank {
  ...
  def draw(g: GdxGraphics, myMaps: Maps): Unit = {
    ...
    g.drawFilledRectangle(posX + cx, myMaps.surface(posX) + cy, length, height, Math.toDegrees(tankAngle).toFloat, Color.GREEN)
