...
class Game(WIDTH: Int = 1920, HEIGHT: Int = 1080)
  ...
  override def onGraphicRender(g: GdxGraphics): Unit = {
    ...
    val cx =
      (-halfHeight * Math.cos(1.57 - myTank.tankAngle)).toFloat
    val cy =
      (halfHeight * Math.sin(1.57 - myTank.tankAngle)).toFloat
    ...
    val dx =
      hTLength * Math.cos(myTank.turretAngle.toRadians)
    val dy =
      hTLength * Math.sin(myTank.turretAngle.toRadians)
    val turretX: Float =
